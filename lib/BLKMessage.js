...
{
	// logger(Log.DEBUG, "new Message firstLine: " + firstLine + " port : " + port + " verb: " + verb
...
{
	logger(Log.DEBUG, "CreateMessage " + port + " " + verb + " " + body)
...
function MessageParser()
{
	...
	function finishedParsing()
	{
		...
		logger(Log.DEBUG, "Parser::finishedParsing ", util.inspect(m))
		_self.emit('msg', m)
		resetForNextMessage()
	}
	...
	function error(text)
	{
		logger(Log.ERROR, "BLKMessage error " + text)
	...
	function parseCharacter(ch)
	{
		...
		if( lineCount == 0 ){
			if( ch == "\n"  ){
				...
				// logger(Log.DEBUG, "nextLine " + currentLine);
				nextLine()
			} else {
				currentLine += ch;
		...
		//	
			...
			if( ch == "\n" ){
				...
				logger(Log.DEBUG, "PARSE destinationPort " + currentLine);
				nextLine()
			}else{
				currentLine += ch;
			}
		...
		//
			...
			if( ch == "\n"  ){
				...
				verb = currentLine;
				logger(Log.DEBUG, "PARSE verb " + currentLine);
				nextLine()
			} else {
				currentLine += ch;
				var len = currentLine.length
		...
		// 
			...
			logger(Log.DEBUG, "Parser body lineCount: " + lineCount + " currentLine: " + currentLine);
			if( ch == "\n" ){
				if( isNaN(parseInt(currentLine, 10)) ){
				...
				logger(Log.DEBUG, "PARSE length " + currentLine + " messageLength"+ msgLength);
				nextLine()
			}else{
				currentLine += ch;
			}
			...
			if( body.length >= msgLength ){
				logger(Log.DEBUG, "Parser body: " + body + " body.length: " + body.length + " msgLength : " + msgLength);
				error("message body too long")
			}
			body += ch;
			if( body.length == msgLength ){
				logger(Log.DEBUG, "PARSE body " + body)
				finishedParsing()
			}
		}
	...
	this.parse = function(buffer){
		...
		logger(Log.DEBUG, "Parser.parse : " + s);
